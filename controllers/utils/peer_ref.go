...
func getPeerRefType(ctx context.Context, c client.Client, peerRef multiclusterv1alpha1.PeerRef, cmNamespace string) (PeerRefType, error) {
	cm, err := FetchClientInfoConfigMap(ctx, c, cmNamespace)
	if err != nil {
...
func IsStorageClientType(ctx context.Context, c client.Client, mirrorPeer multiclusterv1alpha1.MirrorPeer, cmNamespace string) (bool, error) {
	for _, v := range mirrorPeer.Spec.Items {
		peerRefType, err := getPeerRefType(ctx, c, v, cmNamespace)
		if err != nil {
			return false, err
		}
